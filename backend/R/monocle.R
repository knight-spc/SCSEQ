...
timestamp = opt$time
all.markers = read.csv(opt$marker)  ### 这里选择直接读之前的计算结果
...
stdata_subset_list <- lapply(selected_clusters, function(cluster) {
  ...
  # 如果该cluster的细胞数小于5000，则全部保留
  cell_count <- min(300, ncol(subset_data))
  subset_data <- subset(subset_data, cells = sample(colnames(subset_data), cell_count))
  
  # 确保行名存在
  if (is.null(rownames(subset_data))) {
    rownames(subset_data) <- make.names(1:nrow(subset_data))
  }
...
table(stdata_new_MAC@meta.data$annot)
# ============== 3. 准备Monocle输入数据 ==============
# 获取所有层的名称
# layer_names <- grep("^counts", Layers(stdata_new_MAC, assay = "RNA"), value = TRUE)
...
# 更严格的数据过滤
HSMM <- detectGenes(HSMM, min_expr = 5)  # 提高最小表达阈值
HSMM <- HSMM[fData(HSMM)$num_cells_expressed >= 20, ]  # 保留在至少20个细胞中表达的基因

# 使用更稳健的离散度估计方法
HSMM <- estimateDispersions(
  HSMM,
  method = "blind",  # 忽略设计矩阵
  sharingMode = "gene-est-only",  # 仅使用基因特异性估计
  fitType = "local",  # 使用局部拟合
  minmu = 0.1  # 提高最小mu值阈值
)
...
# all.markers
# all.markers = read.csv("all_marker.csv")
...
#                         num_dim = 30,  # 增加维度数以捕捉更多信息
#                         method = 'DDRTree',
#                         norm_method = 'log')  # 确保使用合适的标准化方法
#                         # DDRTree方式！！！！！！！可改ICA
HSMM <- reduceDimension(HSMM, 
                       max_components = 2,
                       num_dim = 30,
...
# 保存图片
ggsave(plot1,file=glue("001.trajectory_{timestamp}.pdf"),width=6,height=6)
ggsave(plot2,file=glue("001.annot_trajectory_{timestamp}.pdf"),width=6,height=6)
ggsave(plot3,file=glue("001.annot_trajectory_split_{timestamp}.pdf"),width=12,height=12)
print("图片保存完成")
all_genes <- fData(HSMM)$gene_short_name
writeLines(all_genes, glue("HSMM_gene_{timestamp}.txt"))
