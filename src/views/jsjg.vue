<template>
  <el-tabs v-model="activeName" @tab-click="handleClickTab">
                ...
                <template #2>
                  ...
                  >
                    <el-button
                      ...
                      text
                      size="small"
                      @click="jumpToTutorial('feature-plot')"
                    >
                      <el-icon>
                        <QuestionFilled />
                      </el-icon>
                  ...
                  <el-select-v2
                    ...
                    lazy
                    filterable
                    :options="marker.gene_list"
                    @change="handleSelectChange_feature"
                    placeholder="Please select"
                    style="width: 240px"
                  />
                  <div ref="chart_feature" style="width: 500px; height: 500px"></div>
                </template>
              </n-split>
            </template>
          </n-split>
    ...
    <el-tab-pane label="细胞通讯分析" name="third">
      <el-row>
        <el-button
          type="primary"
          :loading="cellchat.loading_analysis"
          @click="performCellChatAnalysis"
          style="margin-right: 20px"
        >
          进行细胞通讯分析
        </el-button>
        <el-radio-group v-model="cellchat.weightType">
      ...
      <n-split direction="horizontal" style="height: 100%" default-size="500px">
        <template #1>
          圆图
          <el-tooltip class="box-item" effect="dark" content="介绍手册" placement="top">
            <el-button
              type=""
              text
              size="small"
              @click="jumpToTutorial('cell-communication')"
            >
              <el-icon>
                <QuestionFilled />
              </el-icon>
          ...
          <div style="margin-bottom: 10px">
            <button @click="toggleAllLegends">
              {{ cellchat.allSelected ? "全不选" : "全选" }}
            </button>
          </div>
          <div ref="chart_circle" style="width: 500px; height: 500px"></div>
        </template>
        <template #2>
          热点图
          <el-tooltip class="box-item" effect="dark" content="介绍手册" placement="top">
          ...
          <div class="demo-image__lazy">
            <el-scrollbar
              ref="cellchat_scrollbar"
              style="height: 500px; overflow-y: auto"
            >
              <el-image
                v-for="path in cellchat.heatmapPaths"
                :key="path"
                :src="require('../assets/result/' + path)"
                style="height: 450px; margin: 10px"
                fit="contain"
                lazy
                :scroll-container="cellchat.scrollContainer"
                :preview-src-list="[require('../assets/result/' + path)]"
              >
                <template #toolbar="{ actions, reset }">
                  <el-icon @click="actions('zoomOut')">
    ...
    <el-tab-pane
      label="infercnv"
      name="fourth"
      v-loading="infercnv.loading"
      element-loading-text="处理中..."
      element-loading-background="rgba(0, 0, 0, 0.5)"
      style="position: relative; min-height: 200px"
    >
      <n-split direction="horizontal" style="height: 100%" default-size="580px">
        <template #1>
          <div class="transfer-container">
            <el-transfer
              v-model="infercnv.value"
              :data="infercnv.celllist"
              :titles="['所有细胞', '选中的细胞']"
              @change="pseudotime.value = infercnv.value"
            >
              <template #left-empty>
            ...
            <!-- 添加右侧选项的二次选择区域 -->
            <div class="secondary-selection" v-if="infercnv.value.length">
              <h3>选择参考细胞</h3>
              <el-checkbox-group v-model="infercnv.secondarySelected">
                <el-checkbox v-for="item in rightList" :key="item.key" :label="item.key">
                  {{ item.label }}
                </el-checkbox>
              </el-checkbox-group>
              ...
              <div class="result-display">
                <h4>
                  参考细胞选择结果：<el-tooltip
                    class="box-item"
                    effect="dark"
                    content="被选中的细胞将作为参考集"
                    placement="top"
                    ><el-icon> <InfoFilled /> </el-icon
                  ></el-tooltip>
                </h4>
                <p v-for="item in finalSelection" :key="item.key">
                  {{ item.label }}
                </p>
                <!-- 添加提交按钮 -->
                <el-button
                  type="primary"
                  @click="submitSelections_infercnv"
                  :disabled="!infercnv.value.length || !infercnv.secondarySelected.length"
        ...
        <template #2>
          ...
          <el-tooltip class="box-item" effect="dark" content="介绍手册" placement="top">
            <el-button
              ...
              text
              size="small"
              @click="jumpToTutorial('infercnv-analysis')"
            >
              <el-icon>
                <QuestionFilled />
              </el-icon>
          ...
          <div class="demo-image__lazy">
            <el-image
              :key="imageKey"
              v-if="infercnv.imgPath"
              :src="require('../assets/result/' + infercnv.imgPath)"
              style="width: 100%; max-height: 500px"
              fit="contain"
              :preview-src-list="[require('../assets/result/' + infercnv.imgPath)]"
            >
              <template #toolbar="{ actions, reset }">
                <el-icon @click="actions('zoomOut')">
                  <ZoomOut />
                ...
                <el-icon
                  @click="downloadImage(require('../assets/result/' + infercnv.imgPath))"
                >
                  <Download />
                </el-icon>
              </template>
    ...
    >
      <n-split direction="horizontal" style="height: 100%" default-size="580px">
        <template #1>
          <div class="transfer-container">
            <el-transfer
              v-model="pseudotime.value"
              :data="pseudotime.celllist"
              :titles="['所有细胞', '选中的细胞']"
            >
              <template #left-empty>
                <el-empty :image-size="60" description="No data" />
            ...
            <div class="result-display">
              <el-button
                type="primary"
                @click="submitSelections_pseudotimecells"
                :disabled="!pseudotime.value.length"
                style="margin-top: 15px"
              >
                提交选择结果
              </el-button>
        ...
        <template #2>
          <el-row>
            ...
            <el-tooltip class="box-item" effect="dark" content="介绍手册" placement="top">
              <el-button
                ...
                text
                size="small"
                @click="jumpToTutorial('monocle-analysis')"
              >
                <el-icon>
                  <QuestionFilled />
                </el-icon>
          ...
          <el-button
            type="primary"
            :loading="pseudotime.loading_button"
            @click="QueryLatestResult_pseudotime"
          >
            查询结果
          </el-button>

          <n-split direction="vertical" style="height: 100%" default-size="200px">
            <template #1>
              <div class="demo-image__placeholder">
                <div class="block">
                  <!-- <span class="demonstration">1</span> -->
                  <el-image
                    v-for="path in pseudotime.imgPaths_tra"
                    :key="path"
                    :src="require('../assets/result/' + path)"
                    style="height: 450px; margin: 10px"
                    fit="contain"
                ...
                <div class="block">
                  ...
                  <el-image
                    v-for="path in pseudotime.imgPaths_anno"
                    :key="path"
                    :src="require('../assets/result/' + path)"
                    style="height: 450px; margin: 10px"
                    fit="contain"
            ...
            <template #2>
              <el-select-v2
                v-model="pseudotime.genevalue"
                filterable
                :options="pseudotime.gene_list"
                @focus="fetchPseudoMatchedGenes"
                placeholder="Please select"
              ...
              <el-button
                type="primary"
                :loading="pseudotime.loading_button"
                @click="handleSubmitSelection_pseudogenes"
              >
                生成图表
              </el-button>
              <div
              ...
              >
                ...
                >
                  <el-image
                    v-for="path in pseudotime.imgPaths_gene"
                    :key="path"
                    :src="require('../assets/result/' + path)"
    ...
    >
      <el-row>
        泛癌分析结果
        <el-tooltip class="box-item" effect="dark" content="介绍手册" placement="top">
          <el-button type="" text size="small" @click="jumpToTutorial('tcga-analysis')">
            <el-icon>
              <QuestionFilled />
            </el-icon>
          </el-button>
      ...
      <el-select-v2
        v-model="tcga.genevalue"
        filterable
        :options="tcga.gene_list"
        placeholder="Please select"
      ...
      <el-button
        type="primary"
        :loading="tcga.loading_button"
        @click="handleSubmitSelection_tcga"
      >
        提交选择
      </el-button>
      <el-button
      ...
      <el-button
        type="primary"
        :loading="tcga.loading_button"
        @click="tcga.dialogFormVisible = true"
      >
        设置参数
      </el-button>
      ...
      <div class="photo-gallery">
        <div class="gallery-row">
          <div class="photo-item">
            ...
            <template v-if="tcga.img_boxplot && tcga.img_boxplot.length > 0">
              <el-image
                v-for="path in tcga.img_boxplot"
                :key="path"
                lazy
                :src="require('../assets/result/' + path)"
                :preview-src-list="[require('../assets/result/' + path)]"
          ...
          <div class="photo-item">
            <h3>图片 2</h3>
            <template v-if="tcga.img_survival && tcga.img_survival.length > 0">
              <el-image
                v-for="path in tcga.img_survival"
                :key="path"
                lazy
                :src="require('../assets/result/' + path)"
                :preview-src-list="[require('../assets/result/' + path)]"
        ...
        <div class="gallery-row">
          <div class="photo-item">
            <h3>图片 3</h3>
            <template v-if="tcga.img_meth && tcga.img_meth.length > 0">
              <el-image
                v-for="path in tcga.img_meth"
                :key="path"
                lazy
                :src="require('../assets/result/' + path)"
                :preview-src-list="[require('../assets/result/' + path)]"
          ...
          <div class="photo-item">
            <h3>图片 4</h3>
            <template v-if="tcga.img_TIMER && tcga.img_TIMER.length > 0">
              <el-image
                v-for="path in tcga.img_TIMER"
                :key="path"
                lazy
                :src="require('../assets/result/' + path)"
                :preview-src-list="[require('../assets/result/' + path)]"
      ...
      <el-table
        ...
        style="width: 800px"
      >
        <el-table-column fixed prop="cg" label="cg" width="200"> </el-table-column>
        <el-table-column prop="Correlation" label="Correlation" width="200">
        </el-table-column>
        <el-table-column prop="P_Value" label="P_Value" width="200"> </el-table-column>
        <el-table-column prop="res" label="res" min-width="200"> </el-table-column>
      </el-table>

      <el-button type="primary" class="mt-4" @click="exportToCSV_tcga"
        >Download as CSV</el-button
      >


    <el-dialog v-model="tcga.dialogFormVisible" title="设置参数" width="80%" height="100%">
      <h3 class="form-title">图片一参数设置</h3>
      <el-form ref="tcga.form" :model="tcga.form" label-width="150px" >
            <el-row>
            <el-form-item
              label="离群点大小："
              prop="outlier_size"
            ...
            >
              <el-input v-model.number="tcga.form.outlier_size" style="width: 300px"></el-input>
            </el-form-item>
            <el-form-item label="边框粗细：">
              <el-input v-model="tcga.form.frame_size" style="width: 300px" ></el-input>
            </el-form-item>
            </el-row>
            
            <el-row>
            <el-form-item label="箱线图宽度：">
              <el-input v-model="tcga.form.box_width" style="width: 300px"></el-input>
            </el-form-item>
            <el-form-item label="y轴范围：">
              <el-input v-model="tcga.form.ylimits" style="width: 300px"></el-input>
            </el-form-item>
            </el-row>
            <el-row>
              <el-form-item label="基础字体大小：">
              <el-input v-model="tcga.form.base_size" style="width: 300px"></el-input>
            </el-form-item>
            <el-form-item label="标签字体大小：">
              <el-input v-model="tcga.form.axis_textsize" style="width: 300px"></el-input>
            </el-form-item>
            </el-row>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="tcga.dialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="submitTCGAForm('tcga.form')">确 定</el-button>
      </div>

    </el-dialog>
    </el-tab-pane>
    <el-tab-pane label="genelist" name="seventh">
      <el-select-v2
        v-model="tcga.genevalues"
        filterable
        multiple
        :options="tcga.gene_list"
        placeholder="Please select"
      ...
      <el-button
        type="primary"
        :loading="tcga.loading_button"
        @click="handleSubmitSelection_tcga_list"
      >
        提交选择
      </el-button>
      <el-button type="primary" @click="QueryLatestResult_tcga_list">
        查询结果
      </el-button>
      ...
      <div class="photo-gallery">
        <div class="gallery-row">
          <div class="photo-item">
            ...
            <template v-if="tcga.img_heatmap && tcga.img_heatmap.length > 0">
              <el-image
                v-for="path in tcga.img_heatmap"
                :key="path"
                lazy
                :src="require('../assets/result/' + path)"
                :preview-src-list="[require('../assets/result/' + path)]"
          ...
          <div class="photo-item">
            ...
            <el-scrollbar
              ref="el_scrollbar"
              style="height: 310px; overflow-y: auto; width: 100%"
            >
              <template v-if="tcga.img_forest && tcga.img_forest.length > 0">
                <el-image
                  v-for="(path, index) in tcga.img_forest"
                  :key="path"
                  lazy
                  :src="require('../assets/result/' + path)"
                  :preview-src-list="
                    tcga.img_forest.map((p) => require('../assets/result/' + p))
                  "
                  :initial-index="index"
                  fit="contain"
                ...
                >
                  <template #toolbar="{ actions, reset, activeIndex }">
                    ...
                    <el-icon
                      @click="
                        downloadImage(
                          require('../assets/result/' + tcga.img_forest[activeIndex])
                        )
                      "
                    >
                      <Download />
...
export default {
  ...
  data() {
    return {
      ...
      cellchat: {
        myChart: null,
        allSelected: true, // 默认全选状态
        graph: null,
        links: [],
        realselected: {},
        heatmapPaths: [], // 存储热点图路径
        scrollContainer: null, // 滚动容器引用
        weightType: "count", // 添加新属性，默认为 count
  ...
  watch: {
    // 添加对 weightType 的监听
    "cellchat.weightType": function (newVal, oldVal) {
      if (newVal !== oldVal) {
        // 当 weightType 变化时，重新获取数据和初始化图表
    ...
    "infercnv.value"() {
      this.infercnv.secondarySelected = [];
    },
  },
  computed: {
    rightList() {
      return this.infercnv.celllist.filter((item) =>
        this.infercnv.value.includes(item.key)
      );
    },
    finalSelection() {
      return this.rightList.filter((item) =>
        this.infercnv.secondarySelected.includes(item.key)
      );
    },
    uploadHeaders() {
      return {
  ...
  methods: {
    async loadTaskData(type, taskId) {
      switch (type) {
        ...
        case 'Marker':
          ...
          break;
        case 'Feature':
          ElMessage.success("即将展示Feature数据，在此之前需要先加载最近一次的注释结果");
          await Promise.resolve(this.getUmapData());
          await this.fetchFeatureFigure(); // 然后执行 fetchFeatureFigure
          break;
        case 'Go':
          this.fetchGoFigure();
          break;
        case 'CellChat':
          ...
          break;
        case 'InferCNV':
          this.getInfercnvFigure();
          break;
        case 'Pseudotime':
          this.QueryLatestResult_pseudotime();
          break;
        case 'Pseudotime_gene':
          this.QueryLatestResult_pseudotime();
          break;
        case 'TCGA':
          ...
          break;
        case 'TCGA_genes':
          this.QueryLatestResult_tcga_list();
          break;
      }
    ...
    // 进行得到具体的数据并绘制特征图
    // 获取特征图数据
    async handleSelectChange_feature(value) {
      const formData = new FormData();
      formData.append("gene", value);

      try {
        // 确保DOM已经渲染完成
        await this.$nextTick();
        
        if (!this.$refs.chart_feature) {
          throw new Error('图表容器不存在');
        }

        // 如果已存在实例，先销毁
        if (this.marker.chart) {
          this.marker.chart.dispose();
        }

        // 初始化图表实例
        this.marker.chart = markRaw(echarts.init(this.$refs.chart_feature));
        this.marker.chart.showLoading();

        const res = await post("get_figure_feature", formData);
        this.marker.gene_expression = res.data.gene_expression;
        this.marker.clustered_data = this.cluster_figure.clustered_data;
        this.marker.imgPaths = res.data.img_list;

        // 调用渲染函数
        await this.renderFeatureChart();
      } catch (error) {
        console.error("获取数据失败:", error);
        ElMessage.error(error.message || "获取基因特征图数据失败，请稍后重试");
      } finally {
        if (this.marker.chart) {
          this.marker.chart.hideLoading();
        }
      }
    },
    async fetchFeatureFigure() {
      const formData = new FormData();
      formData.append("taskId", this.task_id);
      ...
      try {
        ...
        const res = await post("fetch_feature_figure", formData);
        this.marker.gene_expression = res.data.gene_expression;
        this.marker.clustered_data = this.cluster_figure.clustered_data;
        this.marker.imgPaths = res.data.img_list;
        ...
        console.error("获取数据失败:", error);
        ElMessage.error(error.message || "获取特征基因图数据失败，请稍后重试");
      } finally {
        if (this.marker.chart) {
          this.marker.chart.hideLoading();
        }
    ...
    // 处理数据并渲染图表
    async renderFeatureChart() {
      try {
        // 遍历数据，按标签分组数据点
        const pointsByLabel = {};
        this.cluster_figure.legend_label = []; // 重置标签列表

        this.marker.clustered_data.forEach((row, index) => {
          const label = row[this.annotation.currunt_model];

          if (!pointsByLabel[label]) {
            pointsByLabel[label] = [];
            this.cluster_figure.legend_label.push(label);
          }
          pointsByLabel[label].push([
            parseFloat(row["umap_data.umap_1"]),
            parseFloat(row["umap_data.umap_2"]),
            parseFloat(this.marker.gene_expression[index]),
          ]);
        });

        // 计算每个聚类的中心点
        this.marker.clusters = Object.keys(pointsByLabel).map((label) => ({
          label,
          center: pointsByLabel[label]
            .reduce((acc, [x, y, z]) => [acc[0] + x, acc[1] + y, acc[2] + z], [0, 0, 0])
            .map((sum) => sum / pointsByLabel[label].length),
          points: pointsByLabel[label],
        }));

        // 输出数据统计
        console.log("基因表达数据统计：", {
          最大值: Math.max(...this.marker.gene_expression),
          最小值: Math.min(...this.marker.gene_expression),
          平均值: this.marker.gene_expression.reduce((a, b) => a + b, 0) / this.marker.gene_expression.length,
          非零值数量: this.marker.gene_expression.filter(v => v > 0).length,
          总数据点: this.marker.gene_expression.length
        });
        ...
        const option = {
          ...
          visualMap: {
            min: 0,
            max: 5,
            // dimension: this.marker.gene_expression,
            orient: "vertical",
            right: 10,
            top: "center",
            text: ["HIGH", "LOW"],
            ...
            formatter: function (param) {
              try {
                ...
                if (param.componentType === "markPoint") {
                  // 使用中心点的 z 值
                  let centerZ = param.data.coord[2]; // 获取中心点的 z 值
                  return centerZ ? centerZ.toFixed(3) : "";
                }
          ...
          dataZoom: [
            // 添加dataZoom组件，支持鼠标滚轮缩放
            // {
            //   type: 'inside', // 内部型数据区域缩放，通过鼠标滚轮缩放
        ...
        yAxis: {
          ...
          // 为每个聚类创建一个散点图系列
          series: this.marker.clusters.map((cluster, index) => ({
            name: cluster.label,

            type: "scatter", // 图表类型为散点图
            data: cluster.points, // 数据点
            symbolSize: 4, // 适当增加数据点的大小以提高可见性
            itemStyle: {},
            ...
            //     {
            //       name: cluster.label,
            //       coord: cluster.center, // 指定标记点的坐标
            //       symbol: "circle",
            //       symbolSize: 3,
            //       itemStyle: {
            //         color: "#D3D3D3", // 标记点的颜色
            //         // borderWidth: 2, // 标记点边框的宽度
            //         // borderColor: 'black' // 标记点边框的颜色
            //       },
            //       label: {
            //         show: true,
            //         formatter: cluster.label, // 显示标记点的名称
            //         position: "center", // 标签的位置
            //         // color: 'yellow', // 确保标签文字在缩放时清晰可见
            //         fontSize: 9, // 适当增加字体大小
            //       },
            //     },
            //     // ... 可以添加更多标记点
        ...
        this.marker.chart.setOption(option, true);
      } catch (error) {
        console.error("渲染图表失败:", error);
        ElMessage.error("渲染图表失败，请稍后重试");
      }
    ...
    // 添加全选/全不选方法
    toggleAllLegends() {
      if (!this.cellchat.myChart) return;

      const option = this.cellchat.myChart.getOption();
      const legendData = option.legend[0].data;
      const legendSelected = {};
      const showlegend = {};

      const graph = this.cellchat.graph;
      // 切换全选状态
      this.cellchat.allSelected = !this.cellchat.allSelected;

      // 为所有图例设置相同的选中状态
      legendData.forEach((name) => {
        // legendSelected[name] = this.cellchat.allSelected;
        showlegend[name] = true;
        this.cellchat.realselected[name] = this.cellchat.allSelected;

        // 更新类别颜色
        const categoryIndex = graph.categories.findIndex(
          (category) => category.name === name
        );
        console.log(categoryIndex);
        if (categoryIndex !== -1) {
          if (!this.cellchat.allSelected) {
            // 保存原始颜色（如果没有保存过）
            if (!graph.categories[categoryIndex].originalColor) {
              graph.categories[categoryIndex].originalColor =
                graph.categories[categoryIndex].itemStyle.color;
            }
            // 设置为灰色
            graph.categories[categoryIndex].itemStyle.color = "#cccccc";
          } else {
            // 恢复原始颜色
            if (graph.categories[categoryIndex].originalColor) {
              graph.categories[categoryIndex].itemStyle.color =
      ...
      // 更新连线的显示状态
      const newLinks = this.cellchat.links.map((link) => {
        const newLink = { ...link };
        newLink.lineStyle = {
          ...link.lineStyle,
          opacity: this.cellchat.allSelected ? 1 : 0,
        };
        return newLink;
      });
      this.cellchat.links = newLinks;

      // 更新图表
      this.cellchat.myChart.setOption({
        animation: false,
        legend: { selected: showlegend },
        series: [
          {
            links: this.cellchat.links,
            data: graph.nodes,
    ...
    // 进行分析
    async performCellChatAnalysis() {
      this.cellchat.loading_analysis = true;
      try {
        const response = await post("perform_cellchat_analysis", {
          weightType: this.cellchat.weightType,
        });

        if (response.data.success) {
          ElMessage.success("细胞通讯分析完成");
          await this.initCircleChart();
          this.getCellChatHeatmaps();
        } else {
          ElMessage.error(response.data.message || "分析失败");
        }
      } catch (error) {
        console.error("细胞通讯分析失败:", error);
        ElMessage.error("分析过程出错，请稍后重试");
      } finally {
        this.cellchat.loading_analysis = false;
      }
    },

    async initCircleChart() {
      this.cellchat.myChart = echarts.init(this.$refs.chart_circle);
      this.cellchat.myChart.showLoading();

      try {
        const response = await get("get_circle_chart_data", {
          params: {
            weightType: this.cellchat.weightType,
            taskId: this.task_id ,
          },
        ...
        this.cellchat.graph = response.data;
        console.log(this.cellchat.graph)
        this.cellchat.graph.nodes.forEach((node) => {
          node.label = {
            show: true,
            position: "right",
            formatter: "{b}",
        ...
        this.cellchat.graph.links.forEach((link) => {
          link.lineStyle = {
            width: link.value,
          };
        });

        this.cellchat.links = this.cellchat.graph.links;

        // 为每个类别生成唯一的颜色
        const categoryCount = this.cellchat.graph.categories.length;
        this.cellchat.graph.categories.forEach((category, index) => {
          const hue = ((index * 360) / categoryCount) % 360;
          category.itemStyle = {
            color: `hsl(${hue}, 70%, 50%)`,
          };
        });
        ...
        this.cellchat.myChart.setOption(option);
        this.cellchat.realselected = this.cellchat.myChart.getOption().legend[0].selected;
        const showlegend = this.cellchat.myChart.getOption().legend[0].selected;

        const legendData = option.legend[0].data;
        // 为所有图例设置相同的选中状态
        legendData.forEach((name) => {
          this.cellchat.realselected[name] = true;
        });

        // 添加图例点击事件处理
        this.cellchat.myChart.on("legendselectchanged", (params) => {
          // 更新图例的选中状态
          this.cellchat.realselected[params.name] = !this.cellchat.realselected[
            params.name
          ]; // 切换选中状态

          const showSelected = this.cellchat.myChart.getOption().legend[0].selected;
          console.log("节点选中状态", this.cellchat.realselected);
          console.log("图例选中状态——showSelected", showSelected);
          ...
          // 更新类别的颜色，选中时变为灰色
          if (categoryIndex !== -1) {
            if (!this.cellchat.realselected[params.name]) {
              // 保存原始颜色（如果没有保存过）
              if (!graph.categories[categoryIndex].originalColor) {
                graph.categories[categoryIndex].originalColor =
                  graph.categories[categoryIndex].itemStyle.color;
          ...
          const newLinks = this.cellchat.links.map((link) => {
            const newLink = { ...link };
            if (params.name === link.source) {
              if (link.lineStyle.opacity === 0) {
                newLink.lineStyle = {
                  ...link.lineStyle,
                  opacity: 1,
                };
              ...
              var opacity = newLink.lineStyle.opacity;
              newLink.lineStyle = {
                ...link.lineStyle,
                opacity: opacity,
              };
          ...
          this.cellchat.links = newLinks;
          showlegend[params.name] = true;

          showSelected[params.name] = !showSelected[params.name]; // 切换选中状态
          this.cellchat.graph = graph;
          // 更新图表，更新图例和连线
          this.cellchat.myChart.setOption({
            animation: false,
            legend: { selected: showSelected },
            series: [
              {
                links: this.cellchat.links,
                data: graph.nodes,
                categories: graph.categories, // 更新类别以应用新的颜色
              },
            ],
          });
        });
      } catch (error) {
        console.error("获取圆形图数据失败:", error);
        this.$message.error("获取圆形图数据失败，请稍后重试");
      } finally {
        this.cellchat.myChart.hideLoading();
      }
    },

    // 获取细胞通讯热点图
    async getCellChatHeatmaps() {
      try {
        const response = await get("get_cellchat_heatmaps", {
          params: {
            weightType: this.cellchat.weightType,
            taskId: this.task_id ,
          },
        ...
        console.log("获取热点图成功:", response.data);
        this.cellchat.heatmapPaths = response.data.img_list;
      } catch (error) {
        console.error("获取热点图失败:", error);
        if (error.response) {
          console.error("请求失败:", error.response.status);
          console.error("错误响应数据:", error.response.data);
          ElMessage.error(error.response.data.message || "获取热点图失败");
        } else {
          ElMessage.error("获取热点图失败: " + error.message);
        }
      }
    },
    ...
    async getCellList() {
      try {
        ...
        const response = await get("get_cell_list");
        if (response.status === 200) {
          const celllist = response.data.celllist || [];
          // 直接映射数组数据
          const annolist = celllist.map((item, index) => ({
            key: item,
            label: item,
            value: item,
          }));
          this.infercnv.celllist = annolist;
          this.pseudotime.celllist = annolist;
          this.SignificanceQC.citiescellOptions = celllist;
          this.go.clusters = annolist
        }
      } catch (error) {
        this.$message.error("获取数据失败：" + error.message);
        this.infercnv.celllist = []; // 出错时清空列表
      }
    },

    async submitSelections_infercnv() {
      // 添加loading状态
      ...
      // });
      this.infercnv.loading = true; // 开始loading
      try {
        const response = await post("submit_selections", {
          firstSelection: this.infercnv.value,
          secondSelection: this.infercnv.secondarySelected,
        });
        ElMessage.success("选择结果提交成功");
        this.infercnv.imgPath = response.data.imgPath || "";
        this.imageKey += 1;
        console.log("this.infercnv.imgPath", this.infercnv.imgPath);
      } catch (error) {
        const errorMsg = error.response?.data?.message || error.message || "未知错误";
        ElMessage.error("提交失败：" + errorMsg);
        this.infercnv.imgPath = "";
      } finally {
        this.infercnv.loading = false;
        console.log("this.infercnv.imgPath", this.infercnv.imgPath);
      }
    ...
    // 获取infercnv热点图
    async getInfercnvFigure() {
      try {
        const response = await get("get_infercnv_figure", {
          params: {
            taskId: this.task_id ,
          },
        });
        
        ElMessage.success("获取infercnv图成功");
        this.infercnv.imgPath = response.data.imgPath || "";
        this.imageKey += 1;
        console.log("this.infercnv.imgPath", this.infercnv.imgPath);
      } catch (error) {
        console.error("获取infercnv图失败:", error);
        if (error.response) {
          console.error("请求失败:", error.response.status);
          console.error("错误响应数据:", error.response.data);
          ElMessage.error(error.response.data.message || "获取infercnv图失败");
        } else {
          ElMessage.error("获取infercnv图失败: " + error.message);
        }
      }
    },
    ...
    // 拟时序分析
    // 选择细胞群
    async submitSelections_pseudotimecells() {
      this.pseudotime.loading = true;
      try {
        const response = await post("submit_pseudotime_cells", {
          selection: this.pseudotime.value,
        });
        this.$message.success("选择结果提交成功");
        this.pseudotime.imgPaths_tra = response.data.imgPaths_tra || [];
        this.pseudotime.imgPaths_anno = response.data.imgPaths_anno || [];
        console.log("this.pseudotime.imgPaths_tra", this.pseudotime.imgPaths_tra);
      } catch (error) {
        const errorMsg = error.response?.data?.message || error.message || "未知错误";
        this.$message.error("提交失败：" + errorMsg);
        this.pseudotime.imgPaths_tra = [];
        this.pseudotime.imgPaths_anno = [];
      } finally {
        this.pseudotime.loading = false;
        console.log("this.pseudotime.imgPaths_tra", this.pseudotime.imgPaths_tra);
      }
    },
    // 获取基因列表
    async fetchPseudoMatchedGenes() {
      try {
        const response = await get("get_pseudo_genes");
        this.pseudotime.gene_list = response.data.gene_list;
      } catch (error) {
        console.error("获取pseudo基因列表失败:", error);
        ElMessage.error("获取pseudo基因列表失败：" + (error.response?.data?.message || error.message));
      }
    },
    // 选择基因
    async handleSubmitSelection_pseudogenes() {
      if (!this.pseudotime.genevalue || this.pseudotime.genevalue.length === 0) {
        ElMessage.warning("请至少选择一个选项");
        return;
      }

      this.pseudotime.loading_button = true;
      try {
        const response = await post("selection_pseudotime_genes", {
          selected_items: this.pseudotime.genevalue,
        });
        this.pseudotime.imgPaths_gene = response.data.imgPaths_gene || [];
        this.imageKey += 1; // 更新key强制刷新图片
        console.log("this.pseudotime.imgPaths_gene", this.pseudotime.imgPaths_gene);
      } catch (error) {
        const errorMsg = error.response?.data?.message || error.message || "未知错误";
        ElMessage.error("提交失败：" + errorMsg);
        this.pseudotime.imgPaths_gene = [];
      } finally {
        this.pseudotime.loading_button = false;
        console.log("this.pseudotime.imgPaths_tra", this.pseudotime.imgPaths_tra);
      }
    },

    async QueryLatestResult_pseudotime() {
      try {
        const response = await get("query_latest_result_pseudotime", {
          params: {
            taskId: this.task_id ,
          },
        });
        ...
        console.error("Error fetching the latest result:", error);
        this.pseudotime.imgPaths_tra = [];
        this.pseudotime.imgPaths_anno = [];
        this.pseudotime.imgPaths_gene = [];
      }
    ...
    // TCGA
    async handleSubmitSelection_tcga() {
      if (!this.tcga.genevalue || this.tcga.genevalue.length === 0) {
        ElMessage.warning("请选择一个选项");
        return;
      }

      this.tcga.loading_button = true;
      try {
        const response = await post("selection_tcga_gene", {
          selected_items: this.tcga.genevalue,
          query: false,
          taskId: this.task_id ,

        });
        this.tcga.img_boxplot = response.data.img_boxplot || [];
        this.tcga.img_survival = response.data.img_survival || [];
        this.tcga.img_meth = response.data.img_meth || [];
        this.tcga.img_TIMER = response.data.img_TIMER || [];
        this.tcga.tableData = response.data.tabledata || [];
        console.log("this.tcga.img_boxplot", this.tcga.img_boxplot);
      } catch (error) {
        const errorMsg = error.response?.data?.message || error.message || "未知错误";
        this.$message.error("提交失败：" + errorMsg);
        this.tcga.img_boxplot = [];
        this.tcga.img_survival = [];
        this.tcga.img_meth = [];
        this.tcga.img_TIMER = [];
        this.tcga.tableData = [];
      } finally {
        this.tcga.loading_button = false;
        console.log("this.tcga.img_boxplot", this.tcga.img_boxplot);
      }
    },
    async handleSubmitSelection_tcga_list() {
      if (!this.tcga.genevalues || this.tcga.genevalues.length === 0) {
        ElMessage.warning("请选择一个选项");
        return;
      }
      ...
      this.tcga.loading_button = true;
      try {
        const response = await post("selection_tcga_genes", {
          selected_items: this.tcga.genevalues,
          query: false,
          taskId: this.task_id ,

        });
        this.tcga.img_heatmap = response.data.img_heatmap || [];
        this.tcga.img_forest = response.data.img_forest || [];
        console.log("this.tcga.img_heatmap", this.tcga.img_heatmap);
      } catch (error) {
        const errorMsg = error.response?.data?.message || error.message || "未知错误";
        this.$message.error("提交失败：" + errorMsg);
        this.tcga.img_heatmap = [];
    ...
    async QueryLatestResult_tcga() {
      try {
        ...
        // 检查后端返回的错误信息
        const msg = error.response?.data?.message || error.message || "未知错误";
        if (msg.includes("缺少文件")) {
          this.$message.error(msg);
        } else {
          this.$message.error("获取TCGA最新结果失败：" + msg);
        }
        this.tcga.img_boxplot = [];
        this.tcga.img_survival = [];
        this.tcga.img_meth = [];
    ...
    async QueryLatestResult_tcga_list() {
      try {
        const response = await post("selection_tcga_genes", {
          query: true,
          taskId: this.task_id ,
        ...
        this.tcga.img_heatmap = response.data.img_heatmap || [];
        this.tcga.img_forest = response.data.img_forest || [];
        console.log("this.tcga.img_forest", this.tcga.img_forest);
      } catch (error) {
        console.error("Error fetching the latest result:", error);
        this.tcga.img_heatmap = [];
        this.tcga.img_forest = [];
    ...
    exportToCSV_tcga() {
      // 检查 tableData 是否存在且为数组
      if (!Array.isArray(this.tcga.tableData)) {
        console.error("tableData 不是一个数组");
        return;
      }

      let csvContent = "cg,Correlation,P_Value,res\n";
      this.tcga.tableData.forEach((row) => {
        // 确保每一行都有必要的属性
        csvContent += `${row.cg || "NA"},${row.Correlation || "NA"},${
          row.P_Value || "NA"
        },${row.res || "NA"}\n`;
      });
  ...
  mounted() {
    this.getQCFigure();
    // this.handleSelectChange_marker(2);
    // this.getDotPlotFigure(2);
  ...
  // this.$nextTick(async () => {
  //   await this.initCircleChart();
  //   await this.getCellChatHeatmaps();
  // });
    this.getCellList();  // 暂时放在页面跳转函数handleClickTab里执行
  },
